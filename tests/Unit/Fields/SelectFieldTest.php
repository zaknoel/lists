...
it('clearMemo() является публичным статическим методом на Select', function () {
    expect(method_exists(Select::class, 'clearMemo'))->toBeTrue();
...
it('Select использует трейт FilterQueryCache', function () {
    $traits = class_uses_recursive(Select::class);

    expect($traits)->toContain(FilterQueryCache::class);
});
