<div class="col-auto mb-3">
    ...
    <div class="dropdown-menu p-3 filter_dropdown" style="border: 1px solid #cecece" aria-labelledby="dropdownMenuButton{{$field->attribute}}"
         >
        <form style="min-width: 200px" data-name="{{$field->attribute}}"
              onsubmit="event.preventDefault(); this.querySelector('button[type=submit]').click(); return false;"
              data-type="{{$field->getType()}}-{{$field->componentName()}}">
            <h6 class="mb-2">{{$field->showLabel()}}:</h6>
            {!! $field->filterContent() !!}
            <div class="text-center mt-3">
                <button type="submit" class="btn btn-sm btn-success" onclick="zakFilter.doFilter(this)">Применить</button>
            </div>
        </form>
    </div>
</div>
